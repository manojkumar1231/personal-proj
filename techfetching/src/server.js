...
app.use(async (req, res) => {
  ...
  if (cookie && cookie.indexOf('accessToken=') > -1) {
    const cookies = cookie.split(';');
    for (let i = 0; i < cookies.length; i += 1) {
      if (cookies[i].indexOf('accessToken=') > -1) {
        console.log(cookies[i].split('accessToken=')[1]);
        providers.client.setJwtToken(cookies[i].split('accessToken=')[1]);
      }
    }
  }
